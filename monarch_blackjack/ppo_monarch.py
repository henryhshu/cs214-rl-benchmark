...
class RolloutWorker(Actor):
    def __init__(
        ...
        if local_env:
            ...
            self._local_env = LocalEnv(game_name)
        else:
...
def _spawn_mesh(num_gpu_workers: int, num_cpu_workers: int = 1):
    if DEVICE.type == "cuda":
        ...
        # This lets multiple workers share each GPU, matching Ray's behaviour.
...
async def main() -> None:
    ...
    if args.local_env:
        ...
        _probe = _LocalEnv(args.game_name)
        obs, legal_actions = _extract_obs_legal(_probe.reset())
    else:
        _configure_openenv_imports()
